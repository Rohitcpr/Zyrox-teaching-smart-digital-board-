...
import { BRAND, GLASS, TEXT } from '../../constants/colors';
...
export const FloatingToolbar: React.FC<Props> = ({ onToolSelect, onCloseRef, onLayerPress, onColorPickerPress }) => {
  ...
  const closeMenu = () => {
    Animated.spring(fadeAnim, { toValue: 0, useNativeDriver: true }).start(() => { setIsOpen(false); setSubPanel('none'); });
    closeAllPanels();
  };

  useEffect(() => { onCloseRef?.(closeMenu); }, []);

  const openMenu = () => { setIsOpen(true); Animated.spring(fadeAnim, { toValue: 1, useNativeDriver: true }).start(); };
  ...
  const handleTool = (t: ToolType) => { setTool(t); closeMenu(); onToolSelect?.(); };
    ...
    <Animated.View style={[styles.wrapper, { transform: [{ translateX: pan.x }, { translateY: pan.y }] }]}>
      ...
      {isOpen && (
        <Animated.View style={[styles.menu, menuPosition, { width: menuWidth, opacity: fadeAnim, transform: [{ scale: fadeAnim.interpolate({ inputRange: [0,1], outputRange: [0.92,1] }) }] }]}>
          ...
          <View style={styles.row}>
            {TOOLS.map((t) => (
              <TouchableOpacity key={t.id} onPress={() => handleTool(t.id)}
                ...
                <Text style={[styles.chipLabel, { color: tool === t.id ? t.color : TEXT.disabled }]}>{t.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <View style={styles.row}>
            ...
            <TouchableOpacity onPress={() => handleSub('size', toggleSizeSlider)} style={[styles.chip, subPanel === 'size' && styles.activeChip]}>
            ...
            <TouchableOpacity onPress={() => handleSub('opacity', toggleOpacitySlider)} style={[styles.chip, subPanel === 'opacity' && styles.activeChip]}>
            ...
            <TouchableOpacity onPress={() => handleSub('grid', toggleGridPanel)} style={[styles.chip, subPanel === 'grid' && styles.activeChip]}>
              <Ionicons name="grid" size={14} color={subPanel === 'grid' ? BRAND.primaryLight : TEXT.secondary} />
              <Text style={styles.chipLabel}>Grid</Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={() => handleSub('shape', toggleShapePanel)} style={[styles.chip, subPanel === 'shape' && styles.activeChip]}>
            ...
            <TouchableOpacity onPress={() => handleSub('bg', toggleBgPanel)} style={[styles.chip, subPanel === 'bg' && styles.activeChip]}>
      ...
      <View {...panResponder.panHandlers}>
        <TouchableOpacity
          ...
          style={[styles.fab, { width: fabSize, height: fabSize, borderRadius: fabSize / 2, opacity: fabOpacity, backgroundColor: isOpen ? BRAND.primaryDark : BRAND.primary, borderColor: currentTool?.color ?? BRAND.primaryLight }]}
        >
          {isOpen ? <Ionicons name="close" size={fabSize * 0.42} color="#fff" /> : <Ionicons name={(currentTool?.icon ?? 'pencil') as any} size={fabSize * 0.42} color="#fff" />}
        </TouchableOpacity>
      </View>
    </Animated.View>
  );
...
const styles = StyleSheet.create({
  ...
  chip: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 9, paddingVertical: 7, borderRadius: 9, borderWidth: 1, borderColor: 'rgba(255,255,255,0.10)', backgroundColor: 'rgba(255,255,255,0.05)' },
