...
export const TopBar: React.FC<Props> = ({ pageId = 'page_001', onImportPress, onLayerPress }) => {
  ...
  const MENU_ITEMS = [
    ...
    { icon: 'resize-outline',          label: 'Ruler',            color: '#F97316', onPress: () => { setShowMenu(false); setShowRuler(true); } },
    { icon: 'radio-button-on-outline', label: 'Spotlight',        color: '#A855F7', onPress: () => { setShowMenu(false); setShowSpotlight(true); } },
    { icon: 'analytics-outline',       label: 'Protractor',       color: '#06B6D4', onPress: () => { setShowMenu(false); setShowProtractor(true); } },
    ...
    { icon: 'layers-outline',          label: 'Layers',           color: '#8B5CF6', onPress: () => { setShowMenu(false); onLayerPress?.(); } },
...
const styles = StyleSheet.create({
  ...
  menuBackdrop: { ...StyleSheet.absoluteFillObject, zIndex: 98 },
  dropdown: {
